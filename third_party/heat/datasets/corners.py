...
class CornersDataset(Dataset):
    ...
    def get_corner_labels(self, corners):
        labels = np.zeros((self.image_size, self.image_size), dtype=np.float32)

        for i, corner in enumerate(corners):
            x = corner[0]
            y = corner[1]

            xint = int(np.floor(float(x)))
            yint = int(np.floor(float(y)))

            xint = max(0, min(xint, self.image_size - 1))
            yint = max(0, min(yint, self.image_size - 1))
