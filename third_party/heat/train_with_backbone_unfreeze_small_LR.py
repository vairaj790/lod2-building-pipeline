...
def main():
    ...
    corner_params = [p for p in corner_model.parameters() if p.requires_grad]
    edge_params = [p for p in edge_model.parameters() if p.requires_grad]

    optimizer = torch.optim.AdamW(
        [
            {"params": backbone_params, "lr": args.lr * 0.1},
            {"params": corner_params, "lr": args.lr},
    ...
    all_params = backbone_params + corner_params + edge_params
    start_epoch = args.start_epoch

    if args.resume:
        ckpt = torch.load(args.resume)
        # Safely load available keys
        if 'backbone' in ckpt:
            backbone.load_state_dict(ckpt['backbone'])
            print("✅ Loaded backbone weights")
        if 'corner_model' in ckpt:
            corner_model.load_state_dict(ckpt['corner_model'])
            print("✅ Loaded corner model weights")
        if 'edge_model' in ckpt:
            edge_model.load_state_dict(ckpt['edge_model'])
            print("✅ Loaded edge model weights")

        print("ℹ️ Stage-2 fine-tuning: skipping optimizer and scheduler state from checkpoint")

        if 'epoch' in ckpt:
            print(f"ℹ️ Pretrained model was trained until epoch {ckpt['epoch']}")
            start_epoch = 0  # ✅ Reset for fine-tuning
            print("⏳ Fine-tuning: restarting from epoch 0")
        else:
            print("ℹ️ Starting fine-tuning from scratch (no epoch info)")
            start_epoch = 0
    ...
    log_dir = os.path.join(args.output_dir, "tensorboard_logs", datetime.now().strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(start_epoch, args.epochs):
        train_stats = train_one_epoch(
            ...
            optimizer, epoch, args.clip_max_norm, args)
        lr_scheduler.step()
